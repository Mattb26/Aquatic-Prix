...
public class IA_Enemigo_Disparo3 : MonoBehaviour
{
    [Header("Ajustes")]
    //public int tiempo;
    public float speed;
    ...
    private GameObject FindGameObjectWithTagg3;
   ...
   // 
    ...
    void Update()
    {
FindGameObjectWithTagg3 = (GameObject.FindGameObjectWithTag("Player3"));


        if (FindGameObjectWithTagg3.layer == 0)               
         {
        //tiempo += 1;
        if(Estado == 1)
        {
        ...
        if (Espera == true)
        {
            ...
            if (Vector3.Distance(GameObject.FindGameObjectWithTag("Player3").transform.position, transform.position) <= distancePlayer)
            {
                Estado = 2;
            }
            RutinaEnemigo();
        ...
        else if (ataque == true)
        {
            ...
            agent.SetDestination(GameObject.FindGameObjectWithTag("Player3").transform.position);
            if (Vector3.Distance(GameObject.FindGameObjectWithTag("Player3").transform.position, transform.position) > distancePlayer)
            {
                Estado = 1;
            }
            ...
            Vector3 posNoRot = new Vector3(GameObject.FindGameObjectWithTag("Player3").transform.position.x, 0.0f, GameObject.FindGameObjectWithTag("Player3").transform.position.z);
            transform.LookAt(posNoRot);
            distanceToTarget = Vector3.Distance(transform.position, GameObject.FindGameObjectWithTag("Player3").transform.position);
            if (Vector3.Distance(GameObject.FindGameObjectWithTag("Player3").transform.position, transform.position) < distancePlayer)
            {
                agent.SetDestination(GameObject.FindGameObjectWithTag("Player3").transform.position);
            }
