...
public class IA_Enemigo_Disparo4 : MonoBehaviour
{
    [Header("Ajustes")]
    //public int tiempo;
    public float speed;
    ...
    private GameObject FindGameObjectWithTagg4;
   ...
   // 
    ...
    void Update()
    {
FindGameObjectWithTagg4 = (GameObject.FindGameObjectWithTag("Player4"));


        if (FindGameObjectWithTagg4.layer == 0)               
         {
        //tiempo += 1;
        if(Estado == 1)
        {
        ...
        if (Espera == true)
        {
            ...
            if (Vector3.Distance(GameObject.FindGameObjectWithTag("Player4").transform.position, transform.position) <= distancePlayer)
            {
                Estado = 2;
            }
            RutinaEnemigo();
        ...
        else if (ataque == true)
        {
            ...
            agent.SetDestination(GameObject.FindGameObjectWithTag("Player4").transform.position);
            if (Vector3.Distance(GameObject.FindGameObjectWithTag("Player4").transform.position, transform.position) > distancePlayer)
            {
                Estado = 1;
            }
            ...
            Vector3 posNoRot = new Vector3(GameObject.FindGameObjectWithTag("Player4").transform.position.x, 0.0f, GameObject.FindGameObjectWithTag("Player4").transform.position.z);
            transform.LookAt(posNoRot);
            distanceToTarget = Vector3.Distance(transform.position, GameObject.FindGameObjectWithTag("Player4").transform.position);
            if (Vector3.Distance(GameObject.FindGameObjectWithTag("Player4").transform.position, transform.position) < distancePlayer)
            {
                agent.SetDestination(GameObject.FindGameObjectWithTag("Player4").transform.position);
            }
